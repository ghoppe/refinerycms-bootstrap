        ...
        content_tag(list_item_tag, :class => menu_item_css(menu_item, index)) do
          ...
          if menu_item_children(menu_item).present?
            if menu_item.depth == 0
              buffer << link_to(raw("#{menu_item.title} #{menu_item_children(menu_item)} <span class=\"caret\"></span>"), context.refinery.url_for(menu_item.url), :class => 'dropdown-toggle', 'data-toggle' => 'dropdown')
              buffer << render_menu_items(menu_item_children(menu_item), 'dropdown-menu')
            else
              buffer << link_to(raw("#{menu_item.title}"), context.refinery.url_for(menu_item.url), :class => 'trigger right-caret')
              buffer << render_menu_items(menu_item_children(menu_item), 'dropdown-menu sub-menu')
            end
          else
